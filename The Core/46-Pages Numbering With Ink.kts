...
fun pagesNumberingWithInk(current: Int, numberOfDigits: Int): Int {
    var current = current
    var currentNumber = current.toString().length
    var numberOfDigits = numberOfDigits


    while(numberOfDigits >= currentNumber){
        numberOfDigits-=currentNumber
        currentNumber= (++current).toString().length
    }
    return current - 1
}
